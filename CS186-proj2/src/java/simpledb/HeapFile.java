 ...
 * constructor.
 * HeapFile是存储元组集合的DbFile的实现
 * 没有特别的顺序。元组存储在页面上，每个页面都是固定的
 * size，文件只是这些页面的集合。 HeapFile有效
 * 与HeapPage密切合作。 HeapPages中描述了HeapPages的格式
 * 构造函数。
 *
...
public class HeapFile implements DbFile {
    ...
    /**存储一个当前正在遍历的页的tuples迭代器的引用,新建一个类，这样一页一页来遍历*/
    // see DbFile.java for javadocs
